...
However with a deterministic system, it can be guaranteed that for a given input, a multi-threaded program is always executed in a same thread scheduling no matter how many times it runs. In this paper we present Melchior, a deterministic system that guarantees deterministic scheduling for a group of threads/processes. Melchior consists of a compiler part which instruments the program, a user-space utility to launch the deterministic namespace, and a patch to Linux kernel to control the scheduling inside the original Linux scheduler. The mission of Melchior is to control the sequence of inter-thread/process communication, in terms of share memory access and system calls. The compiler part is responsible for finding those communication parts and tags them, then the deterministic scheduler will enforce the sequence of executing those tagged area.

Melchior is implemented with a deterministic scheduling algorithm that is highly optimized for scalability, the runtime overhead with big number of threads (up to 64) is much less than any other existing deterministic systems. We evaluated the implementation with many parallel workloads, including pbzip2.
...
Guide}\cite{Lamport:LaTeX}.trovato@corporation.com
